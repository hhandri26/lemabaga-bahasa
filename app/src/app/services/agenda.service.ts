...
export class AgendaService {
    ...
    getList(params = { month: new Date().getMonth(), year: new Date().getFullYear()}, draw: number = 0, perPage: number = 10):
        Observable<{ pagination: any; items: any[] }> {
        return this._httpClient.get<{ pagination: any; items: any[] }>(this._apiUrl + '/kegiatan/agenda/list', { params }).pipe(
            tap((response: any) => {
                const _items = response.mapData.agenda;
                const firstCut = draw * perPage;
                const secondCut = firstCut + perPage;
                const items = _items.slice(firstCut, secondCut);
                this._items$.next(items);
                this._pagination.next({ recordsTotal: (response.mapData.agenda).length, perPage, draw });
            })
        );
    }
    ...
    save(params): Observable<any> {
        return this._httpClient.post<any>(this._apiUrl + '/kegiatan/agenda/save', params).pipe(
    ...
    delete(id: string): Observable<boolean> {
        return this.items$.pipe(
            take(1),
            switchMap(items => this._httpClient.delete(this._apiUrl + '/kegiatan/agenda/delete/' + id).pipe(

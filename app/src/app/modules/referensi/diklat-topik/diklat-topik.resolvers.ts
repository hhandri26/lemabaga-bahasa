...
{
    constructor(private _referensiDiklatTopikService: ReferensiDiklatTopikService) {
    }

    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<{ pagination: any; items: any[] }> {
        return this._referensiDiklatTopikService.getList();
    }
}
...
export class DetailResolver implements Resolve<any>
{
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> {
        return this._referensiDiklatTopikService.getById(+route.paramMap.get('id'))
            .pipe(
                catchError((error) => {

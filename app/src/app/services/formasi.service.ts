...
export class FormasiService {
    _apiUrl = `${environment.apiUrl}/formasi`;
    ...
    getListComplete(params = { q: '' }, draw: number = 0, perPage: number = 10):
        Observable<{ pagination: any; items: any[] }> {
        return this._httpClient.get<{ pagination: any; items: any[] }>(this._apiUrl + '/formasi-detail/completed', { params }).pipe(
            tap((response: any) => {
    ...
    calculate(body: FormData):
        Observable<any> {
        return this._httpClient.post<any>(this._apiUrl + '/hitung-formula', body, { headers: this.__HTTPHeaderUpload(this._authService.accessToken) });
    }

    createUsul(body: FormData):
        Observable<any> {
    ...
    approveUsulan(body: FormData):
        Observable<any> {
        return this._httpClient.post<any>(this._apiUrl + '/send-recommendation', body, { headers: this.__HTTPHeaderUpload(this._authService.accessToken) });
    }

    sendMenpan(body: FormData):
        Observable<any> {
        return this._httpClient.post<any>(this._apiUrl + '/send-approved-menpan', body, { headers: this.__HTTPHeaderUpload(this._authService.accessToken) });
    }
    ...
    returnUsulan(params):
        Observable<any> {
        return this._httpClient.post<any>(this._apiUrl + '/return-formasi', params).pipe(
    ...
    getDetail(id: number):
        ...
        return this._httpClient.get<any>(this._apiUrl + '/detail/' + id).pipe(
            ...
            switchMap((_item) => {
                if (!_item) {
                    return throwError('Tidak dapat menemukan detail usulan formasi dengan id #' + id + '!');
                }
                return of(_item);
            })
        );

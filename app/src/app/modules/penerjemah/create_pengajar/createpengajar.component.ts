...
@Component({
    templateUrl: './createpengajar.component.html',
...
export class CreateComponentPengajar implements OnInit, OnDestroy {
    isLoading = false;
    form: FormGroup;
    ...
    constructor(
        public matDialogRef: MatDialogRef<CreateComponentPengajar>,
    ...
    if (params.bahasaId) { 
        ...
        this._penerjemahService.saveDataUtamaPengajar(formData).subscribe(
            (result) => {
                if (result?.success) {
                    this._toastr.success('Pengajar berhasil ditambahkan, password akan dikirimkan melalui email', 'Tambah pengguna berhasil');
                    this.form.reset();

    ...
    signIn(credentials: any): Observable<any> {
        ...
        return this._httpClient.post(`${environment.apiUrl}/auth/signin`, credentials).pipe(
            switchMap((response: any) => {
                this._authenticated = response?.success;
                if (response.success) {
                    this.accessToken = response.mapData.data.accessToken;
                    this.role = response.mapData.data.roles[0];
                    this._userService.user = response.mapData.data;
                }

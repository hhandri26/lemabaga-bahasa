...
export class ListComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        this.items$ = this._referensiKegiatanTranslasiService.items$;

        this._referensiKegiatanTranslasiService.item$
        ...
        this._referensiKegiatanTranslasiService.pagination$
            .pipe(takeUntil(this._unsubscribeAll))
        ...
        fromEvent(this.findKeyword.nativeElement, 'keyup')
            .pipe(
                ...
                tap(() => {
                    ...
                    this._changeDetectorRef.markForCheck();
                    this._referensiKegiatanTranslasiService.getList({q: (this.findKeyword.nativeElement.value) ? this.findKeyword.nativeElement.value : ''}, this.draw, this.perPage).subscribe();
                })
            )
            .subscribe();
    ...
    fetch(pageIndex, pageSize) {
        return this._referensiKegiatanTranslasiService.getList({q: (this.findKeyword.nativeElement.value) ? this.findKeyword.nativeElement.value : ''}, pageIndex, pageSize).pipe();
    }
    ...
    create(): void {
        ...
        dialogRef.afterClosed().subscribe((result) => {
            if (result) {
                this._referensiKegiatanTranslasiService.getList().subscribe();
            }
        });
    }

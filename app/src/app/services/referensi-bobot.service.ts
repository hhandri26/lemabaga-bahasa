...
export class ReferensiBobotService {
    ...
    getList(draw: number = 0, perPage: number = 10):
        Observable<{ pagination: any; items: any[] }> {
        // return this._httpClient.get<{ pagination: any; items: any[] }>(this._apiUrl + '/refference/pendidikan', { params }).pipe(
        return this._httpClient.get<{ pagination: any; items: any[] }>('api/apps/reff/bobot').pipe(
            tap((response: any) => {
    ...
    save(params): Observable<any> {
        return this._httpClient.post<any>(this._apiUrl + '/write-reff/pendidikan/save', params).pipe(
    ...
    delete(id: string): Observable<boolean> {
        return this.items$.pipe(
            take(1),
            switchMap(items => this._httpClient.delete(this._apiUrl + '/write-reff/pendidikan/delete/' + id).pipe(

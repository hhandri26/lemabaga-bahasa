...
export class QuizService {
    ...
    getList(page: number = 0, size: number = 1000, search: object = {}):
        ...
        return this._httpClient.post<{ pagination: any; items: any[] }>(this._apiUrl + '/quiz/bucket/filter', JSON.stringify(params), { headers: this.httpHeaders }).pipe(
            tap((response) => {
    ...
    detail(id): Observable<any> {
        return this._httpClient.get<any>(this._apiUrl + '/quiz/bucket/' + id).pipe(
    ...
    addQuestion(params): Observable<any> {
        return this._httpClient.post<any>(this._apiUrl + '/quiz/add-question', JSON.stringify(params), { headers: this.httpHeaders }).pipe(
            tap((result: any) => result),
            switchMap((result) => {
                if (!result) {
    ...
    editQuestion(params): Observable<any> {
        return this._httpClient.post<any>(this._apiUrl + '/quiz/modify-question', JSON.stringify(params), { headers: this.httpHeaders }).pipe(
            tap((result: any) => result),
            switchMap((result) => {
                if (!result) {
                    return throwError('save error!');
    ...
    deleteQuestion(id):
        Observable<any> {
        return this._httpClient.delete<any>(this._apiUrl + '/quiz/delete-question/' + id).pipe(
            tap((result: any) => result),
            switchMap((result) => {
                if (!result) {
                    return throwError('delete error!');
